var LDRY = LDRY || {};

// LDRY framework level urls..
LDRY.fu = 'app/'; // front end url
//LDRY.su = 'http://landrysleap.com/LeadershipConferenceServices.asmx'; // service url
LDRY.su = 'http://landrysleaptest.ldry.com/LeadershipConferenceServices.asmx'; // service url
LDRY.du = 'app/data/'; // data url
//LDRY.ru = 'http://www.landrysleap.com/sponsors/resources/logos/'; // resource url
//LDRY.ru = 'https://farm8.staticflickr.com/'; 

LDRY.ru = 'app/images/resources/sponsors/logos/'; // resource local url
LDRY.lu = 'http://www.landrysleap.com/images/mobile/logos/'; // logos url
LDRY.tu = 'http://www.landrysleap.com/images/mobile/logos/thumbnails/'; // thumbnail url

// LDRY mode
LDRY.mode = 'd'; // p: production, d: development, t: testing

LDRY.init = function(){
 	//console.log('LDRY.init');
	//PG.init();
 	//LDRY.leapApp.init();
}
